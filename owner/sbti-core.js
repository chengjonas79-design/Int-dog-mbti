  ...
  "So3": {
    ...
    function copyShareText() {
      ...
      const text = `测完SBTI我人麻了，我竟然是「${typeName}」\n你是死者、吗喽、还是废物？来测 → ${location.origin}/owner/\n测完自己再测你家毛孩子 → ${location.origin}\n#SBTI #人格测试`;
      const btn = document.getElementById('copyShareBtn');
      const onOk = () => { if(btn){ btn.textContent='已复制，去发朋友圈吧'; setTimeout(()=>{ btn.textContent='📋 一键复制晒圈文案'; },2500); } };
    ...
    (function(){
      ...
      s.src = 'https://static.cloudbase.net/cloudbase-js-sdk/2.17.3/cloudbase.full.js';
      s.async = true;
      s.onload = async function() {
        try {
          var app = cloudbase.init({ env: 'mengchong-dev-6g4qar062ad266bc' });
          await app.auth({ persistence: 'session' }).signInAnonymously();
          var db = app.database();
          window._cbDb = db;
          db.collection('events').add({ c:'funnel', a:'page_view', l:'SBTI人格测试', pet:'owner', t:new Date(), ts:Date.now() });
        } catch(e) { console.warn('[CloudBase]', e); }
      };
